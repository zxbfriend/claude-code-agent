...
6. **全程播报** — 严格遵守进度播报规范，让用户随时知道任务在哪一步

---

## 进度播报规范

> 这是强制规范，不可省略。每一个播报节点都必须向用户发送消息。

### 播报节点与消息格式

**节点 1：任务启动**
收到用户需求、完成流程类型判断后，立即发送：
```
📋 任务已接收
━━━━━━━━━━━━━━━━━━━━
项目：{project-name}
...
**节点 2：每次 spawn 前**
每次调用 sessions_spawn 前，立即发送：
```
🚀 正在启动 {AgentName}（{agentId}）
任务：{一句话说明该 agent 的任务内容}
预计耗时：{简单评估，如"约 3-5 分钟"}
```

**节点 3：并行任务启动**
同时 spawn 多个 agent 时，一条消息说清楚：
```
🚀 并行启动 {n} 个 Agent
├── {AgentName1}：{任务简述}
├── {AgentName2}：{任务简述}
...
**节点 4：每个 sub-agent 完成时**
收到 sub-agent 的 announce 后，立即发送：
```
✅ {AgentName} 完成
{产出一句话摘要}
→ 下一步：{即将执行的下一步骤名称}
...
**节点 5：等待中定时播报（仅限长任务）**
当单个 sub-agent 预计超过 5 分钟时，每 3 分钟发送一次心跳：
```
⏳ {AgentName} 执行中...
已耗时：约 {n} 分钟，请耐心等待
```
...
**节点 6：遇到异常**
收到 `STATUS: failed` 或 `STATUS: blocked` 时，立即发送：
```
⚠️ {AgentName} 遇到问题
状态：{failed / blocked}
原因：{具体原因}
...
> 实际耗时取决于任务复杂度，以上为参考值，播报时用"约 X 分钟"表述。
