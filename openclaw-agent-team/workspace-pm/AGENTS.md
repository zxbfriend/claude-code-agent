...
进度播报是强制行为，每个节点都必须执行，不可因为"任务还在进行"而跳过。

### 播报时机检查表

每次执行以下动作时，必须先发播报消息再执行动作：

```
□ 完成流程类型判断    → 发"任务启动"播报
□ 调用 sessions_spawn → 发"启动 {AgentName}"播报（spawn 前）
□ 收到 announce 回调  → 发"{AgentName} 完成"播报（立即，不延迟）
□ 遇到 failed/blocked → 发"遇到问题"播报（立即，不延迟）
□ 等待超过 3 分钟     → 发"执行中..."心跳播报
```
...
```
1. 准备好 task 描述
2. 发送"🚀 正在启动 {AgentName}"消息 ← 必须先发消息
3. 调用 sessions_spawn
4. 等待 announce
```

**不允许**：先 spawn，完成后才告诉用户"刚才启动了 xxx"。

### 并行 spawn 的播报顺序

```
1. 发送"🚀 并行启动 {n} 个 Agent"消息，列出所有 agent 和各自任务
2. 同时调用多个 sessions_spawn
3. 每收到一个 announce，立即发"✅ {AgentName} 完成"
4. 全部完成后，发"📦 本阶段全部完成，进入下一步"
```
...
- 如果还未收到，发送心跳消息
- 心跳消息格式：`⏳ {AgentName} 执行中...已耗时约 {n} 分钟，请耐心等待`
- 超过 10 分钟未响应，触发超时处理规则

### 播报消息要发到哪里

发给触发本次任务的用户，通过原来的对话渠道（Telegram / Slack / 等），不需要额外配置，直接在当前 session 中回复即可。
