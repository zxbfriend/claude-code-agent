...
7. 不得用需求提出人的人工确认替代 `QAEngineer` 的测试结论；如 QA 无法执行，必须标记为 `blocked` 或记录明确豁免与风险。
...
7. 重要决策记录目录：`D:\Github\openclaw\dev-agents\docs`
8. 决策记录命名格式：`<task_id>-decision-<topic>.md`
...
7. 不得要求需求提出人接替子 Agent 的职责；需要人工输入时，只能请求确认、授权、环境、账号、数据或业务判断，并将缺失项记录为阻塞或风险。

## 11. 记忆、心跳与决策治理

1. 分层记忆系统遵循：`D:\Github\openclaw\dev-agents\shared\protocols\memory-system.md`
2. 心跳检查系统遵循：`D:\Github\openclaw\dev-agents\shared\protocols\heartbeat-system.md`
3. 重大技术、流程、范围、质量门禁或发布取舍，必须使用 `D:\Github\openclaw\dev-agents\templates\decision-template.md` 留痕。
4. 角色记忆不得替代正式文档、阶段报告、交接记录、审查报告或 QA 报告。
